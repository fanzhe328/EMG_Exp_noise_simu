...
def load_gaussion_distribute(subject='subject_1'):
    file_path = root_path + '/result_gaussian/sensitivity/' + subject + '_simulation_1.npy'
    return np.load(file_path)

def gaussian_simu(trains, targets, subject, action_num, chan_num, feat_num):
...
if __name__ == '__main__':
    ...
    for subject in subject_list:
        data = load_gaussion_distribute(subject)
        print data.shape
